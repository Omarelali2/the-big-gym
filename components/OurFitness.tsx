...
const OurFitnessServer = async () => {
  ...
  if (!user)
    return <AccessDenied message='You need to log in to view our tools.' />
  ...
  if (!dbUser)
    return <AccessDenied message='Your account is not in our database.' />

  if (dbUser.isAdmin) {
    return <ToolsSlider userPackageName='Admin' />
  }

  if (!dbUser.selectedPackage || !dbUser.subscriptionActive) {
    return (
      <AccessDenied message='You need an active subscription to access these tools.' />
    )
  }
