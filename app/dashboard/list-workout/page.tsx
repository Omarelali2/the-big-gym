        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {workouts.map((w) => (
            <tr key={w.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-2">{w.muscle.map(m => m.name).join(", ") || "-"}</td>
              <td className="px-4 py-2">
                {w.images.length > 0 ? (
                  <div className="flex gap-2">
                    {w.images.map((img, i) => (
                      <img key={i} src={img} alt={w.name} className="w-10 h-10 rounded-lg object-cover" />
                    ))}
                  </div>
                ) : (
                  "-"
              ...
              <td className="px-4 py-2">{new Date(w.updatedAt).toLocaleDateString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
