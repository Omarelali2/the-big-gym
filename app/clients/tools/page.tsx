...
const tools = [
  ...
  },
  {
    ...
    image: "https://plus.unsplash.com/premium_photo-1689298468802-5c2cfb626971?w=600&auto=format&fit=crop&q=60",
    allowedPackages: ["Standard", "Premium"]
  },
  {
    title: "Macro Calculator",
    description: "Split your calories into carbs, protein, and fats depending on your diet.",
    link: "/clients/tools/macro",
    image: "https://totalshape.com/wp-content/uploads/2023/07/PROTEIN-CALCULATOR-Featured-Image.webp",
    allowedPackages: ["Premium"]
  },
]
...
export default async function ToolsPage() {
  ...
  if (!user) {
    return (
      <div className='flex flex-col items-center justify-center h-[60vh] text-center'>
        ...
        <p className='text-gray-300 text-lg mb-6'>You need to log in to view our tools.</p>
      </div>
    )
  }
  ...
  if (!dbUser || !dbUser.selectedPackage) {
    return (
      <div className='flex flex-col items-center justify-center h-[60vh] text-center'>
        <h2 className='text-3xl md:text-4xl font-extrabold text-red-500 mb-4'>⚠️ Access Denied</h2>
        <p className='text-gray-300 text-lg mb-6'>You need a package to access these tools.</p>
      </div>
    )
  }

  const userPackageName = dbUser.selectedPackage.name
  ...
  return (
    <div className='max-w-7xl mx-auto px-6 py-12'>
      <h1 className='text-5xl sm:text-6xl font-extrabold text-center bg-clip-text text-transparent 
      ...
      {availableTools.length === 0 ? (
        <p className='text-gray-400 text-center'>No tools available for your package.</p>
      ) : (
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
          {availableTools.map((tool, idx) => (
            <div key={idx} className='group relative bg-gray-800 rounded-3xl shadow-xl hover:shadow-2xl transition-shadow duration-500 overflow-hidden flex flex-col'>
