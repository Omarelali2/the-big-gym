...
export default function ChatPage({ params }: { params: Promise<{ coachId: string }> }) {
    ...
    <div className="flex flex-col h-screen mt-15 bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-700 sticky top-0 bg-gray-900 z-10">
        <div className="flex items-center">
          ...
          />
          <span className="text-white font-semibold text-lg">{coachData.name}</span>
        </div>
        <div className="flex space-x-4">
          <button className="text-green-500 hover:text-green-400 text-xl">📞</button>
          <button className="text-blue-500 hover:text-blue-400 text-xl">🎥</button>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-gray-700 bg-gray-900">
        <ChatInput
          ...
          onNewMessage={(msg: Message) =>
            setMessages(prev => [...prev, { ...msg, text: msg.text ?? "", sender: msg.sender }])
          }
        />
      </div>
    </div>
