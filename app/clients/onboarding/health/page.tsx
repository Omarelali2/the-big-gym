  ...
  return (
    <div className="w-120 md:w-full mx-auto mt-12 p-8 bg-gradient-to-br from-gray-600 via-gray-700 to-gray-800 shadow-3xl rounded-3xl text-white">
      ...
      {step === 2 && (
        <div className="flex flex-col gap-4">
          <label className="flex items-center gap-2">
            <Dumbbell className="w-6 h-6 text-indigo-400" /> Enter your weight (kg)
          </label>
          <input
            type="range"
            name="weight"
      ...
      {step === 7 && (
        <div className="flex flex-col gap-4">
          ...
          >
            {loading ? <Loader2 className="animate-spin h-6 w-6" /> : "Finish"}
          </button>
        </div>
      )}

      {message && (
        <p className="mt-5 text-center text-gray-200 font-medium">{message}</p>
      )}
    </div>
  )
}
